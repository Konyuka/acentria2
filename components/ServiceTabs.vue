    ...
    <div>
        <div class="grid md:grid-cols-3 gap-4 mb-5">
            <button v-for="menu in menus" @click="setMenu(menu.tag)"
                ...
                class="text-lg shadow-2xl button-animation inline-flex items-center justify-center rounded-sm transition-colors duration-300 focus:outline-none hover:bg-black hover:text-white focus:ring-brand-primary-dark px-2 py-3">
        ...
        <div :class="[gridClass]" class="container grid grid-cols-1 md:grid-cols-2 gap-4 py-0">
            <NuxtLink :href="item.link" v-for="item in menuOptions"
                class="button-animation border-t-8 border-solid border-black shadow-2xl m-auto flex min-h-fit md:min-h-auto rounded-md bg-black/20 px-5 py-2 w-full h-full">
